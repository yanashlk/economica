...
app.get("/admin/submissions/:id", requireAdmin, async (req, res) => {
    try {
        ...
        const sRes = await pool.query(
            `select s.id, s.status, s.created_at, s.submitted_at, s.reviewed_at, s.reviewed_by, f.title as form_title
       from submissions s
       join forms f on f.id = s.form_id
       where s.id=$1`,
            [id]
...
app.patch("/admin/submissions/:id", requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { answers } = req.body || {};
...
app.post("/admin/submissions/:id/review", requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const pool = getPool();
        ...
        if (s.reviewed_at) {
            ...
            return res.json({ ok: true, reviewed: false, ...upd.rows[0] });
        } else {
            const upd = await pool.query(
                "update submissions set reviewed_at=now(), reviewed_by=$2, updated_at=now() where id=$1 returning reviewed_at, reviewed_by",
                [id, req.user?.uid || null]
            );
            return res.json({ ok: true, reviewed: true, ...upd.rows[0] });
        }
    } catch (e) {
        console.error(e);
        return res.status(500).json({ error: "Server error" });
